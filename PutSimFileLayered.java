...
public class PutSimFileLayered {
	...
	try {
		...
		PotentialFullKai ai2 = new PotentialFullKai();
		...
		long start = System.currentTimeMillis();
		//Score score = new Score();
		float[] ret = ai2.decidePutLocale(stat, col, row, 1);
		if (ret[0] < 0) {
		    return;
		}
		long time = System.currentTimeMillis() - start;
		...
		tsumoList += ai2.tsumoListNum + "\n";
		calcTimes += ai2.scoreCalcNum + "\n";
		str_time += Long.toString(time) + "\n";
		str_score += ret[0] + "\n";
		chain += ret[1] + "\n";
		totalPuyo += Field.getPuyoNum(stat.field) + "\n";
