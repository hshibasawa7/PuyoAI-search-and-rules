...
public class PotentialLayered {
    public int scoreCalcNum = 0;
    public int tsumoListNum = 0;

    //int cnt_scoreCalc = 0;

    public PotentialLayered() {
	System.out.println("using PotentialLayered algorithm.");
    }
	...
	*/
    ...
    //ret = max(score)
    public float[] simulation(int[][] ex_field, int[] ex_height, int[][][] tsumoList, final int old_max_depth, int depth, int[] col, int[] row) {
	...
	int max_depth = tsumoList.length;
	int size = 0;
	int idx = -1;
	float[] avgData = new float[3];
	avgData[0] = .0F;
	avgData[1] = .0F;
	avgData[2] = .0F;
	...
	for (int j = 0; j < tsumoList[depth-1].length; j++) {
	    ...
	    idx = idxList.get(i);
	    float[] data = scoreList.get(i);
	    avgData[0] += data[0];
	    avgData[1] += data[1];
	    avgData[2] += data[2];

	}
	avgData[0] /= tsumoList[depth-1].length;
	avgData[1] /= tsumoList[depth-1].length;
	avgData[2] /= tsumoList[depth-1].length;
