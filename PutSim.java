...
public class PutSim {
	...
	for (int i = 1; i < iMax; i += 2) {
	    ...
	    if ((iMax - i)/2+1 < depth) {
		f_score = ai.decidePutLocale(stat, col, row, (iMax - i)/2+1);
	    } else {
		f_score = ai.decidePutLocale(stat, col, row, depth);
	    }
	    score.score = (int)f_score[0];
	    score.chain = (int)f_score[1];
	    score.totalDel = (int)f_score[2];
